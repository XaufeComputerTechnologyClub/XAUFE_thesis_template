...
\officialDemand{
  ...
  另外，由于使用了文件生成命令，
  编译的时候需要使用命令
  \lstinline{xelatex -output-directory="./build" main.tex}
  指定生成摘要的目标文件夹为\lstinline{./build}
  或使用构建脚本，
  或将\lstinline{\\subfile{sections/abstract}}
  注释掉来忽略摘要部分.
  也可以在
  \lstinline{./build}
  新建
  \lstinline{Chinese_abstract.tex}与
  \lstinline{English_abstract.tex}
}
